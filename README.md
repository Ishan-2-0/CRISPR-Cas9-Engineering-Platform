...
CRISPR-based gene editing involves 3 distinct computational
...
2016 (referenced below). 5 criteria combine into a 100 point score.
...
Gene sequences are frozen to disk on first fetch to avoid rehitting
...
### 1. CRISPR is a 3 problem domain, scoped to Problem 3 only
...
gene. The project UI presents disease to gene context for all 3
...
Initial guide RNA candidates had no principled ranking. 2
...
Doench et al. 2016. Each of the 5 scoring functions defines
...
out of memory oom errors on the RTX 3060. The fix was identical to the
...
zero Cell 2 Scorer documents appearing. The cause was that guide RNA
...
redundant results, preventing the retriever from returning 6 similar
...
caused DMD, CFTR, and BRCA1 to score 0 on the disclaimer metric
...
- End mto end pipeline connecting all three problems
